...
// src/utils/fetch.js
async function getLatestRelease(repo, { headers: headers2, excludePrereleases = true }) {
  try {
    ...
    const release = excludePrereleases ? releases.find((r2) => !r2.prerelease) : releases[0];
...
function updateSection(readmePath, newMarkdown, headerTitle) {
  ...
  const content = fs2.readFileSync(readmePath, "utf8");
  ...
  const header = headerTitle && headerTitle.trim() !== "" ? `${headerTitle.trim()}
...
` : "";
  ...
  fs2.writeFileSync(readmePath, updated);
...
// src/themes/table.js
function renderTable(releases, footerText, actorUrl, includeBody = true, showGlobalFooter = true, dateFormat = "ISO") {
  const rows = releases.map((r2) => {
    const preBadge = r2.prerelease ? " (\u26A0\uFE0F *Pre-release*)" : "";
    const dateStr = formatDate(r2.published_at, dateFormat);
    return `| [${r2.repo}](${r2.html_url}) | ${r2.tag_name}${preBadge} | ${dateStr} |`;
  }).join("\n");
  const detailsSection = includeBody && releases.some((r2) => r2.body) ? "\n\n---\n\n" + releases.filter((r2) => r2.body).map((r2) => {
    const releasesUrl = `https://github.com/${r2.repo}/releases`;
    return [
      `<details>`,
      `<summary><strong>${r2.repo} ${r2.tag_name}</strong> \u2013 Details</summary>`,
      "",
      r2.body.split("\n").slice(0, 20).join("\n"),
      "",
      `[\u{1F517} See all releases \u2192](${releasesUrl})`,
      `</details>`
    ].join("\n");
  }).join("\n\n") : "";
  const footer = showGlobalFooter && footerText && actorUrl ? `
...
// src/themes/list.js
function renderList(releases, footerText, actorUrl, includeBody = true, showGlobalFooter = true, dateFormat = "ISO") {
  const markdown = releases.map((r2) => {
    const preBadge = r2.prerelease ? " \u26A0\uFE0F *Pre-release*" : "";
    const releasesUrl = `https://github.com/${r2.repo}/releases`;
    const dateStr = formatDate(r2.published_at, dateFormat);
    const header = `### [${r2.repo}](${r2.html_url}) \u2014 **${r2.tag_name}**${preBadge}${dateStr ? ` (${dateStr})` : ""}`;
    const bodySection = includeBody && r2.body ? [
      "<details>",
      "<summary>\u{1F50D} View Release Details</summary>",
      "",
      r2.body.split("\n").slice(0, 20).join("\n"),
      "",
      `[\u{1F517} See all releases \u2192](${releasesUrl})`,
...
${bodySection}`.trim();
  ...
  return `${markdown.trim()}${showGlobalFooter ? `

[${footerText}](${actorUrl})` : ""}`;
}

// src/themes/card.js
function renderCard(releases, footerText, actorUrl, includeBody = true, showGlobalFooter = true, dateFormat = "ISO") {
  const markdown = releases.map((r2) => {
    const releasesUrl = `https://github.com/${r2.repo}/releases`;
    const preBadge = r2.prerelease ? " \u26A0\uFE0F *Pre-release*" : "";
    const formattedDate = formatDate(r2.published_at, dateFormat);
    const header = `> \u{1F4A0} **[${r2.repo}](${r2.html_url}) \u2014 ${r2.tag_name}**${preBadge ? `  
> ${preBadge}` : ""}${formattedDate ? ` _${formattedDate}_` : ""}`;
    const body = includeBody && r2.body ? [
      "",
      "> <details>",
      "> <summary>\u{1F4DC} View details</summary>",
      ">",
...
// src/themes/compact.js
function renderCompact(releases, footerText, actorUrl, includeBody = true, showGlobalFooter = true, dateFormat = "ISO") {
  const markdown = releases.map((r2) => {
    const preBadge = r2.prerelease ? " \u26A0\uFE0F *Pre-release*" : "";
    const releasesUrl = `https://github.com/${r2.repo}/releases`;
    const dateStr = formatDate(r2.published_at, dateFormat);
    let entry = `- **[${r2.repo}](${r2.html_url})** \u2014 ${r2.tag_name}${preBadge}${dateStr ? ` (${dateStr})` : ""}`;
    if (includeBody && r2.body) {
      const shortSummary = r2.body.split("\n").filter(Boolean)[0] || "";
...
// src/index.js
var INPUT = (name, def) => process.env[`INPUT_${name.toUpperCase()}`] || def;
var REPOS = INPUT("repos", "").split("\n").map((r2) => r2.trim()).filter(Boolean);
var TOKEN = INPUT("token", "");
var MAX = parseInt(INPUT("max_repos", "5"));
var INCLUDE_BODY = INPUT("include_body", "true") === "true";
var EXCLUDE_PRE = INPUT("exclude_prereleases", "true") === "true";
var THEME = INPUT("theme", "table");
var HEADER_INPUT = INPUT("header_title", "").trim();
var HEADER = HEADER_INPUT ? HEADER_INPUT : "";
var FOOTER = INPUT("footer_link_text", "Explore More \u2192");
var SHOW_GLOBAL_FOOTER = INPUT("show_global_footer") === "true";
var DATE_FORMAT = INPUT("date_format", "ISO");
var SKIP_IF_NO_CHANGE = INPUT("skip_if_no_change", "true") === "true";
var AUTO_COMMIT = INPUT("auto_commit", "false") === "true";
var README = INPUT("readme_path", "README.md");
var ACTOR = process.env.GITHUB_ACTOR || "user";
...
async function run() {
  console.log("\u{1F4D8} Checking README.md for release section markers...");
  const readme = fs3.existsSync(README) ? fs3.readFileSync(README, "utf8") : "";
  if (!readme.includes("<!--LATEST_RELEASES_START-->") || !readme.includes("<!--LATEST_RELEASES_END-->")) {
    console.error(
...
`
  ...
  console.log(`\u{1F50D} Fetching latest releases for ${REPOS.length} repo(s)...`);
  const results = (await Promise.all(
    REPOS.map(
      (r2) => getLatestRelease(r2, { headers, excludePrereleases: EXCLUDE_PRE })
    )
  )).filter(Boolean).slice(0, MAX);
  console.log(`\u{1F4E6} ${results.length} release(s) fetched successfully.`);
  let render;
  switch (THEME) {
    case "list":
      render = renderList;
  ...
  console.log("\u{1F50D} Checking for changes...");
  const LAST_FILE = "latest-releases.json";
  let lastReleases = {};
  if (fs3.existsSync(LAST_FILE)) {
    try {
      lastReleases = JSON.parse(fs3.readFileSync(LAST_FILE, "utf8"));
    } catch {
      console.warn(
  ...
  const newTags = Object.fromEntries(results.map((r2) => [r2.repo, r2.tag_name]));
  ...
  console.log("\u{1F9FE} Updating README.md section...");
  updateSection(README, markdown, HEADER ?? "");
  fs3.writeFileSync(LAST_FILE, JSON.stringify(newTags, null, 2));
  if (AUTO_COMMIT) {
    console.log("\u{1FA84} Auto-commit enabled \u2014 committing README changes...");
    autoCommit(README);
  } else {
...
if (import.meta.url !== `file://${process.argv[1]}` && process.env.DEBUG) {
  console.log("\u{1F9EA} Index.js imported (not auto-running)...");
}
export {
  run
};
